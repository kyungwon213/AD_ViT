...
from .utils.helpers import build_model_with_cfg, named_apply, checkpoint_seq
from .utils.layers import PatchEmbed, Mlp, DropPath, to_2tuple, to_ntuple, trunc_normal_, _assert
from .utils.registry import register_model
from .utils.vision_transformer import checkpoint_filter_fn, get_init_weights_vit
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed_kp = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
            norm_layer=norm_layer if patch_norm else None)
    ...
    def forward_features(self, x, kp_x, camera_id, view_id):
        if_keyp = True
        
        x = self.patch_embed(x)
        kp_x = self.kp_weight * self.patch_embed_kp(kp_x)
        # print (self.sie_embed.shape, camera_id)
        if (if_keyp):
            if self.absolute_pos_embed is not None:
                x = x + self.absolute_pos_embed + kp_x 
            if self.cam_num > 0 and self.view_num > 0:
                # print(type(x), type(self.absolute_pos_embed), type(self.sie_xishu * self.sie_embed[camera_id * self.view_num + view_id]))
                # print(self.sie_embed.shape, camera_id * self.view_num + view_id, camera_id, self.view_num, view_id)
                x = x + self.absolute_pos_embed + self.sie_xishu * self.sie_embed[camera_id * self.view_num + view_id] + kp_x
            elif self.cam_num > 0:
                x = x + self.absolute_pos_embed + self.sie_xishu * self.sie_embed[camera_id] + kp_x
            elif self.view_num > 0:
                x = x + self.absolute_pos_embed + self.sie_xishu * self.sie_embed[view_id] + kp_x
            else:
                x = x + self.absolute_pos_embed + kp_x
        else:
            if self.absolute_pos_embed is not None:
            ...
            if self.cam_num > 0 and self.view_num > 0:
                # print(type(x), type(self.absolute_pos_embed), type(self.sie_xishu * self.sie_embed[camera_id * self.view_num + view_id]))
                # print(self.sie_embed.shape, camera_id * self.view_num + view_id, camera_id, self.view_num, view_id)
                x = x + self.absolute_pos_embed + self.sie_xishu * self.sie_embed[camera_id * self.view_num + view_id]
            elif self.cam_num > 0:
                x = x + self.absolute_pos_embed + self.sie_xishu * self.sie_embed[camera_id]
            elif self.view_num > 0:
                x = x + self.absolute_pos_embed + self.sie_xishu * self.sie_embed[view_id]
            else:
                x = x + self.absolute_pos_embed  
        x = self.pos_drop(x)
        x = self.layers(x)
    ...
    def forward(self, x, kp_x, cam_label, view_label):
        x = self.forward_features(x, kp_x, cam_label, view_label)
