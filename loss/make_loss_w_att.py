            ...
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                if isinstance(score, list):
                    ...
                    if cfg.MODEL.JPM and cfg.MODEL.ATTR:
                        ...
                        ID_LOSS = 0.2 * (JPM_LOSS + ATTR_LOSS) + 0.8 * F.cross_entropy(score[0], target)

                    elif cfg.MODEL.ATTR:
                        ATTR_LOSS = [bce_wl_loss(scor, target_att) for scor in score[1:]]
                        ATTR_LOSS = sum(ATTR_LOSS) / len(ATTR_LOSS)
                        ID_LOSS = ATTR_LOSS + F.cross_entropy(score[0], target)
                else:
                    ID_LOSS = F.cross_entropy(score, target)
                ...
                if isinstance(feat, list):
                    if cfg.MODEL.JPM and cfg.MODEL.ATTR:
                        TRI_LOSS = [triplet(feats, target)[0] for feats in feat[1:]]
                        TRI_LOSS = sum(TRI_LOSS) / len(TRI_LOSS)
                        TRI_LOSS = 0.2 * TRI_LOSS + 0.8 * triplet(feat[0], target)[0]
                    elif cfg.MODEL.ATTR:
                        ATTR_TRI_LOSS = triplet(feat[1], target)[0]
                        TRI_LOSS = 0.2 * ATTR_TRI_LOSS + 0.8 * triplet(feat[0], target)[0]
                else:
                    TRI_LOSS = torch.nn.triplet(feat, target)[0]
