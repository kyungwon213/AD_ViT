...
class LTCC(BaseImageDataset):
    ...
    def __init__(self, root='', verbose=True, **kwargs):
        ...
        if cfg.MODEL.Evaluate == "ClothChangingSetting":
            train = self._process_dir('cloth-change_id_train.txt', relabel=True)
            query = self._process_dir('cloth-change_id_query.txt', relabel=False)
            gallery = self._process_dir('cloth-change_id_test.txt', relabel=False)
        elif cfg.MODEL.Evaluate == "StandardSetting":
            train = self._process_dir('cloth-unchange_id_train.txt', relabel=True)
            query = self._process_dir('cloth-unchange_id_query.txt', relabel=False)
            gallery = self._process_dir('cloth-unchange_id_test.txt', relabel=False)
    ...
    def _process_dir(self, list_path, relabel=False):
        datasets = []
        ...
        for file_info, attribute in zip(file_paths, attributes):
            ...
            file_info = os.path.join(self.dataset_dir, file_info.split('/')[-2], img_info)
            kp_file_info = os.path.join(self.dataset_dir, "keypoints", file_info.split('/')[-2], img_info)
            datasets.append((file_info, kp_file_info, pid, camid, 1, self.unique_clothes_dict[unique_clothid], attributes_info))
