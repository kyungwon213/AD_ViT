...
class NKUP(BaseImageDataset):
    ...
    def _process_dir(self, dir_path, list_path, relabel=False):
        ...
        for img_info, attribute in zip(list_path, attributes):
            ...
            unique_clothid = int(img_info.split('_')[1].split("D")[1])    # represents recording day which can also be considered clothes id
            ...
            kp_file_info = os.path.join(self.dataset_dir, "keypoints", dataset_type, img_info)
            if relabel: pid = pid2label[pid]  # train ids must be relabelled from zero
            datasets.append((img_path, kp_file_info, pid, camid, 1, self.unique_clothes_dict[unique_clothid], attributes_info))
